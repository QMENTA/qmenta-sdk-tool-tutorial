...
class QmentaSDKToolMakerTutorial(Tool):
    def tool_inputs(self):
        ...
        self.add_info(
            "Required inputs:<br><b>&bull; Oncology medical image</b>: 3D image to analyze<br>&ensp;" \
        )
        ...
        self.add_heading("Radiomic feature classes:")
    ...
    def run(self, context):
        ...
        fname1 = self.inputs.input_images.c_image1[0].file_path
        ...
        if "do_segmentation" in self.inputs.perform_steps:
            ...
            ants.plot(img_seg['segmentation'], filename="segmentation.png")
        ...
        if "do_thickness" in self.inputs.perform_steps:
            ...
            img.plot(overlay=thickimg, overlay_cmap='jet', filename="thickness.png")
...
def run(context):
    Qmenta-sdk-tool-maker-tutorial().tool_outputs()  # this can be removed if no results configuration file needs to be generated.
    Qmenta-sdk-tool-maker-tutorial().run(context)